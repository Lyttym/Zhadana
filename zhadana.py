	...
	while True:
		try:
			...
			if heyZhadano:
				...
				with sr.Microphone() as source:
					ZhadanaSay('Здійснюю прибирання шумів, зачекай будь ласка')
					r.adjust_for_ambient_noise(source, duration=5)
					ZhadanaSay('Шуми успішно прибрано')
			elif r.recognize_google(audio, language='uk-UA').lower() == "жадана видалити дані":
				ZhadanaSay('Ти справді хочеш видалити всі свої дані?')
				print('Скажіть "Підтвердити", щоб підтвердити, "Скасувати", щоб відхилити')
				with sr.Microphone() as source:
					audio = r.listen(source)
				if r.recognize_google(audio, language='uk-UA').lower() == 'підтвердити':
					os.remove('config.ini')
					os.remove('zhadana.mp3')
					ZhadanaSay('Дані успішно видалено')
				if r.recognize_google(audio, language='uk-UA').lower() == 'скасувати':
					ZhadanaSay('Видалення даних скасовано')
